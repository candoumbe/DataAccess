namespace Candoumbe.DataAccess.Tests.Repositories
{
    ...
    public class FirstAsyncTests : EntityFrameworkRepositoryTestsBase, IClassFixture<SqliteDatabaseFixture>
    {
        ...
        [Fact]
        public async Task Given_hero_exists_and_has_an_acolyte_When_calling_FirstAsync_without_including_acolytes_Then_result_should_not_have_acolyte()
        {
            // Arrange
            ...
            // Act
            Hero actual = await repository.FirstAsync(predicate: x => x.Id == hero.Id, cancellationToken: default).ConfigureAwait(false);
        ...
        [Fact]
        public async Task Given_hero_exists_and_has_an_acolyte_When_calling_FirstAsync_with_including_acolytes_Then_result_should_not_have_acolyte()
        {
            // Arrange
            Hero hero = new Hero(Guid.NewGuid(), Faker.Person.FullName);
            Acolyte acolyte = new Acolyte(Guid.NewGuid(), Faker.Person.FullName);
            ...
            // Act
            Hero actual = await repository.FirstAsync(x => x.Id == hero.Id,
