...
public class FirstOrDefaultTests : EntityFrameworkRepositoryTestsBase, IClassFixture<SqliteDatabaseFixture>, IAsyncLifetime
{
    ...
    public async Task Given_hero_exists_and_has_an_acolyte_When_calling_FirstOrDefaultAsync_without_including_acolytes_Then_result_should_not_have_acolyte()
    {
        ...
        FilterSpecification<Hero> expression = new(x => x.Id == hero.Id);

        // Act
        Option<Hero> maybeHero = await repository.FirstOrDefault(expression);
        ...
        maybeHero.Match(
            ...
            _ => throw new UnreachableException($"'{nameof(EntityFrameworkRepository<Hero, SqliteStore>.FirstOrDefault)}' must return the entity when it exists")
#else
                _ => throw new NotSupportedException($"'{nameof(EntityFrameworkRepository<Hero, SqliteDbContext>.FirstOrDefault)}' must return the entity when it exists")
#endif
        );
    }
    ...
    public async Task Given_hero_exists_and_has_an_acolyte_When_calling_FirstOrDefaultAsync_with_including_acolytes_Then_result_should_not_have_acolyte()
    {
        ...
        // Act
        Option<Hero> maybeHero = await repository.FirstOrDefault(predicate,
                                                                 [IncludeClause<Hero>.Create(x => x.Acolytes.Where(item => item.Name == acolyte.Name))]);

        // Assert
        maybeHero.Match(
            h => h.Acolytes.Should()
                .HaveSameCount(h.Acolytes, $"'{nameof(Hero.Acolytes)}' was explicitely included").And
